...
const normalizeResource = resource => {
  // normalize & get copy
  const definition = lodash.isString(resource) ? { wellknown: resource } :
...
const createGetSchemaHttpAction = resource => {
  ...
  // build action
  const endpoint = `/${lodash.toLower(wellknown.plural)}/schema`;
  const action = {
...
const createGetSingleHttpAction = resource => {
  ...
  const action = {
    [methodName]: id => {
      ...
      const endpoint = `/${lodash.toLower(wellknown.plural)}/${id}`;
      return get(endpoint, params).then(response => response.data);
    },
  };
...
const createPutHttpAction = resource => {
  ...
  const action = {
    [methodName]: payload => {
      ...
      const defaults = (resource.params || {}).filter;
      const data = lodash.merge({}, defaults, payload);
      const endpoint = `/${lodash.toLower(wellknown.plural)}/${idOf(data)}`;
 ...
 * @public
 */
const SHORTCUTS = lodash.merge({}, PARTIES_SHORTCUTS);
