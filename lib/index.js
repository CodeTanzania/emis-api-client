...
const mapBetween = between => {
  ...
  if (isBetween) {
    const span = lodash.merge({}, between);
    let upper = lodash.max(distinct(span.to, span.from));
    let lower = lodash.min(distinct(span.from, span.to));
    upper = moment(upper)
      .utc()
      .endOf('date')
      .toDate();
...
const mapRange = range => {
  ...
  if (isRange) {
    const span = lodash.merge({}, range);
    const upper = lodash.max(distinct(span.max, span.min));
    const lower = lodash.min(distinct(span.min, span.max));
    return {
      $gte: lower,
      $lte: upper,
    };
...
const prepareParams = params => {
  // clone params
  const options = lodash.merge({}, params);
