...
const normalizeResource = resource => {
  // get copy
  const definition = lodash.merge({}, resource);

  // rormalize shortcut
  const { shortcut } = definition;
  let singular = inflection.singularize(shortcut);
  let plural = inflection.pluralize(shortcut);
  definition.shortcut = { singular, plural };

  // rormalize wellknown
  const { wellknown } = definition;
  singular = inflection.singularize(wellknown);
  plural = inflection.pluralize(wellknown);
